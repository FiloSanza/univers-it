...
<x-app-layout id='post-page'>
    ...
    <div class="border-b-2 border-gray-600 mt-4 p-4">
        ...
        @if($post->image_id)
            <div class="mx-auto w-4/5 mt-3">
                <img class="mx-auto max-w-full h-auto rounded-xl" src='{{ route('image.get', $post->image_id) }}' alt="Post Image" />
            </div>
        @endif

        <div class="mt-2 grid grid-cols-2 mx-auto w-full sm:w-1/3">
            <div 
                @if(Auth::check() && Auth::user()->hasVerifiedEmail())
                    id="like-button" 
                @endif
                class="my-1 mx-2 p-2 flex flex-row inline-block rounded-xl align-right {{ !(Auth::check() && Auth::user()->hasVerifiedEmail()) ? 'bg-gray-300' : ''}}"
            >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 bg-transparent">
                ...
                <span id="like-number" class="text-right w-4/5 bg-transparent">  </span>
            </div>
            <div 
                @if(Auth::check() && Auth::user()->hasVerifiedEmail())
                    id="dislike-button" 
            ...
            >
                ...
                <span id="dislike-number" class="text-right w-4/5 bg-transparent">  </span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-row justify-between font-bold">        
        ...
        @if(Auth::check() && Auth::user()->hasVerifiedEmail())
        <div class="m-1 mx-2 p-2 cursor-pointer bg-blue-100 inline-block rounded-xl align-right lg:mx-0 hover:bg-gray-300" data-target='comment-modal'>
